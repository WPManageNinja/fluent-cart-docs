...
Copy the snippet below **near the top** of your `wp-config.php` (before the `/* That's all, stop editing! */` line) and replace the stars with your real Cloudflare values:

```php
define( 'FLUENT_CART_R2_SETTINGS', serialize( array(
    'provider'          => 'cloudflare',
    'account-id'        => '********************',
    'access-key-id'     => '********************',
    'secret-access-key' => '****************************************',
...
Once FluentCart detects this constant, the credential fields lock automatically and the Connection Method card confirms the keys were picked up from `wp-config.php`. The in-app snippet includes a **Copy** button to make this paste-in friendlier.

> **Heads up:** to later switch to a different method here, comment out or remove the `FLUENT_CART_R2_SETTINGS` define from `wp-config.php` and reload the page.
...
define( 'FLUENT_CART_R2_SETTINGS', serialize( array(
    ...
    'access-key-id'     => 'EXAMPLEACCESSKEYID12',
    'secret-access-key' => 'EXAMPLE_SECRET_ACCESS_KEY_1234567890abcd',
) ) );
```
...
**I defined `FLUENT_CART_R2_SETTINGS` but the UI still shows empty fields.**
* Confirm the constant is in `wp-config.php` (not inside a plugin or theme file) and loads before FluentCart.
* Confirm the array structure matches the example exactly — keys are `provider`, `account-id`, `access-key-id`, `secret-access-key`.
