...
Copy the snippet below **near the top** of your `wp-config.php` (before the `/* That's all, stop editing! */` line) and replace the stars with your real AWS keys:

```php
define( 'FLUENT_CART_S3_SETTINGS', serialize( array(
    'provider'          => 'aws',
    'access-key-id'     => '********************',
    'secret-access-key' => '****************************************',
) ) );
...
Once FluentCart detects this constant, the credential fields in the UI are automatically locked, and the Connection Method card will tell you the keys have been picked up from your `wp-config.php`.

> **Heads up:** to later switch to a different method here, comment out or remove the `FLUENT_CART_S3_SETTINGS` define from `wp-config.php` and reload the page.
...
define( 'FLUENT_CART_S3_SETTINGS', serialize( array(
    'provider'          => 'aws',
    'access-key-id'     => 'AKIAEXAMPLEKEYID1234',
...
**I defined `FLUENT_CART_S3_SETTINGS` but the UI still shows empty fields.**
* Confirm the constant is in `wp-config.php` (not in a plugin file) and loads before FluentCart.
* Confirm the array structure matches the example exactly — keys are `provider`, `access-key-id`, `secret-access-key`.
