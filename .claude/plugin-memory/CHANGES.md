...
## v1.6.4 — Sep 10, 2026
- **Trigger:** user pasted a 10-line "Newly Added" list from the 1.6.3/1.6.4 cycle and asked for verification against source, a placement plan, and the doc edits.
- **Range:** core `1.6.2..HEAD` (`9aca779a1`, `development`, `FLUENTCART_VERSION 1.6.4`) — 134 commits / 390 files. Pro at `0e08b8c2` (`development`, also 1.6.4). **All ten features are core-side.** Pro's window in this range is Paddle MoR reconciliation, Mollie/Authorize.Net currency fixes, and R2 bucket resolution: no doc impact for this run.
- **All ten changelog lines VERIFIED in source.** Commit map, for the next audit:
  - one-time upgrade UI → `5d42f7b08` (PR #2671). `CustomerOrderController::orderDetails` stamps `has_upgrade_paths` per item; gate is `payment_type === 'onetime'` **AND** no `line_meta.parent_item_id` (bundle children excluded) **AND** a `variant_upgrade_path` meta row on the variation **AND** order payment status in paid / partially paid / partially refunded. `resources/public/customer-profile/Vue/SingleOrder.vue:144` mounts the existing `UpgradePlan` with `button-text` "Upgrade Plan"; dialog title is "Upgrade Options", table columns Package / Price / Amount to pay / Action.
  - staging protection → `aa8e55315` + `6b8e2a0d2` + `f0f70526b` (PR #2674). Store setting `subscription_mode_guard`, default `yes`, **fail-closed** (only a literal `'no'` disables). `SubscriptionHelper::canProcessInMode()` compares order mode against `order_mode`. Covers renewal-invoice creation, `SystemChargeService::executeCharge`, `reconcileProcessingCharge`, and **both** reminder services. Held work re-arms a daily re-check. **UI does exist** despite no hit in `resources/`: it is schema-driven from `api/StoreSettings.php:1321` (`subscription_mode_guard_grid`), on the **Subscriptions** settings tab, labelled "Staging Protection" / "Don't bill live subscriptions from this site while it is in test mode".
  - Bricks Sale + Sold Out badges → `2e0d56ff9` + `52cef9807` (PR #2407). Core `Modules/Templating/Bricks/Elements/ProductsCollection.php` (**not** the `fluent-cart-bricks-blocks` addon). Content-tab controls `showSaleBadge`, `saleBadgeText` (default "Sale!"), `saleBadgeShowPercentage`, `saleBadgePercentageText` (`-{percent}%`), `saleBadgePriceSource` (Default Variant | Best Discount (All Variants)), `saleBadgeShape` (Badge | Ribbon), `saleBadgePosition` (4 corners); sold-out mirror with default text "Out of Stock". Style-tab twins carry Typography / Background Color / Text Color. Render logic in `BricksHelper::renderProductBadges()`.
  - Bricks Default Filter → `cb99d817b` + `3b3b75768` + `f363d2eba` (PRs #2377, #2378). Same file. `defaultFilterEnabled`, `defaultFilterAllowOutOfStock`, `defaultFilterWildcard` (preset search), plus one searchable multi-select per taxonomy. **Ignored when `Is main query` is on** — that caveat is in the control's own description string.
  - Stripe hosted submit label + itemised lines → `dad1b3e42`, `b6e010a7f`, `787f32f3b`, `0d9e69b59` (PR #2792). New `submit_type` gateway setting, label "Submit Button Label", options auto/pay/book/donate/subscribe, **hosted-only**. `787f32f3b` narrowed it per mode: payment sessions accept `auto|book|donate|pay`, subscription sessions accept `auto|donate|subscribe`, anything else is simply not sent. `buildHostedLineItems()` emits per-item + shipping + additive tax and **declines to a single aggregate line whenever the parts do not reconcile exactly** to the charge amount; a Stripe validation rejection also retries as the aggregate. Cap `MAX_HOSTED_LINE_ITEMS = 90`.
  - Cart block → `7c21ad0d4` (PR #2638). `fluent-cart/cart` container plus `cart-items` / `cart-total` / `cart-checkout-button` children, all rendering through `CartRenderer` so output matches `[fluent_cart_cart]`. Editable text via two new filters `fluent_cart/cart/total_label` (default "Total") and `fluent_cart/cart/checkout_button_text` (default "Go to Checkout"); empty attribute means "keep the default". **Styles tab deliberately empty**; supports only `html => false` and `align: wide|full`. Empty-cart state lives on the container.
  - three-stage overdue reminders → `e67b7a0de` (PR #2641). Notifications `renewal_overdue_first` / `_followup` / `_final`, group "Scheduler / Reminder Actions", titles "First overdue renewal reminder to customer" / "Follow-up overdue renewal reminder to customer" / "Final overdue renewal notice to customer". Stage is mapped by **position in the admin's day list** (min → first, max → final, between → followup), not by hardcoded cutoffs. Setting `renewal_reminder_overdue_days`, default `1,3,7`, schema in `app/Hooks/Handlers/ReminderHandler.php:332` under the Reminders page's **Invoice Reminders** group. Legacy `fluent_cart/renewal_reminder_overdue` survives as a `do_action_deprecated` bridge (since 1.6.3) flagged `staged_email_dispatched` so it does not double-send.
  - daily report chart grouping → `dec3797f5` (PR #2631). **Landed inside the 1.6.2 window but was never documented**, which is why it shows up on this changelog. `ReportHelper::sanitizeGroupKey()` gained `'daily'`; `resources/admin/Modules/Reports/Utils/useGroupKeyOptions.js` is shared by all six line charts (Order, Revenue, Refund, Subscription, Product, Customer). Auto's label is computed client-side from the date range (≤91d Daily, ≤365d Monthly, else Yearly) and collapses to a plain "Auto" while an explicit grouping is selected. The month/year-only restriction in `SubscriptionReportController` is on `getCohortData`, **not** the subscription line chart: Daily works on all six.
  - consent-gated attribution → `d928e906d` (PR #2696). `UTMManager` dispatches cancellable `fluent_cart_utm_before_store` on `window`; a provider claims it with `preventDefault()` then answers `detail.allow()` / `detail.deny()`. Deny also clears stored data (Art. 7(3) withdrawal). **Unclaimed = unchanged behaviour**, which is the majority case. Globals: `window.fluentCartUtmManager` (`FluentCartApp.js:162`) with `hasConsent()` / `revokeConsent()`. **Core ships the gate only** — no banner, no settings UI, no consent-plugin bridge in either clone. Documented as developer-facing accordingly; do not claim a shipped integration.
  - developer filters → `de9602d2b` (#48) adds `fluent_cart/checkout/order_data` in **both** `CheckoutProcessor` and `AdminOrderProcessor` (same hook name, `args` differs); `b0ee76bf0` (PR #2763) adds `fluent_cart/email_notification/mailer` at send time in `mailEmailsOfEvent()` and `mailByEmailName()`, falling back to the original mailer on any non-Mailer return. Mailer's real API is `addCC` / `addBCC` / `setFrom` / `setReplyTo` / `setSubject` / `body` / `addAttachment` / `setIsHtml` — **there is no `bcc()`**, an early draft of the snippet got this wrong.
- **Doc pages updated (18 files, no new pages, no sidebar change):**
  - `guide/customer-dashboard/purchase-history.md` — new "Upgrading a One-Time Purchase" section
  - `guide/product-types-creation/defining-upgrade-paths.md` — new "Upgrades on One-Time Purchases" subsection with the three eligibility gates
  - `guide/product-types-creation/store-managed-subscriptions.md` — new "Keeping Staging Sites from Billing Customers" section; Renewal Reminders paragraph rewritten for the configurable, escalating schedule
  - `guide/settings-configuration/store-settings.md` — Store Mode now points at staging protection
  - `guide/customization-and-themes/fluentcart-bricks-blocks.md` — new Default Filter, Sale Badge, Sold Out Badge control groups; "three groups" intro corrected
  - `guide/customization-and-themes/using-gutenberg-blocks.md` — new "21. Cart" block section
  - `guide/payments-checkout/connecting-payment-gateways/stripe-settings.md` — Step 3 extended with Submit Button Label and the itemised hosted line items
  - `guide/settings-configuration/email-configuration/reminders.md` — new "Configuring Invoice Reminders" section + "How the Three Stages Escalate"
  - `guide/settings-configuration/email-configuration/configuring-email-notification.md` — three staged emails added to the reminder list
  - `guide/reporting-analytics/reports-dashboard-overview.md` — new "Choosing How a Trend Chart Is Grouped" section
  - `guide/reporting-analytics/{orders,revenue,refunds,subscription,product,customer}-report.md` — one "Chart grouping" bullet each, linking to the section above
  - `guide/reporting-analytics/order-sources-report.md` — new "Consent and stored attribution" subsection
  - `guide/customization-and-themes/code-snippets.md` — three new snippets: order-data filter, new "Emails" H2 with the mailer filter, consent-gating under Attribution
- **Plugin changes flagged but skipped (no doc impact):** FluentPlayer product-video gallery and video-tab redesign (a separate companion plugin's surface; existing `fluentplayer-integration.md` untouched pending a dedicated pass); product reviews and ratings (`27462f682` — **large user-facing feature with no doc page at all**, see open questions); a long run of security hardening (FC-SEC-03/04/05/09/11, bulk-order-delete gating, storage-path containment); zero-decimal currency fixes across Stripe/PayPal; grace-period and reactivation decoupling; translation/text-domain sweeps; N+1 storefront perf work.
- **Build status:** `npm run docs:build` clean. No `featured:generate` run — no pages added and no H1 changed.
- **Open questions carried forward:**
  - **`guide/changelog.md` was NOT updated.** The user supplied only the "Newly Added" list, with no Improvements, no Fixes, and no release date, and the changelog template needs all three code-groups. Ask for the full 1.6.3 + 1.6.4 changelog before writing that entry.
  - **Product reviews and ratings (`27462f682`) is undocumented.** Shipped in this window, no page exists. Propose a new page under `guide/product-types-creation/` or `guide/store-management/`.
  - Screenshots: none of the 13 new sections carry images. Highest value captures: Staging Protection setting, Stripe Submit Button Label, Bricks Sale/Sold Out badge panels, Cart block in the editor, Invoice Reminders panel, report chart grouping dropdown, Upgrade Plan button on a one-time order.
  - Still outstanding from v1.6.2: the seven unverifiable 1.6.2 changelog lines, the Elementor Blocks / Migrator / Customer Rights addon cards, and the `store-managed-subscriptions` old-slug 404 redirect.
