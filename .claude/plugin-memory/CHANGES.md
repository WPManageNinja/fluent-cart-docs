...
- **Confirmed merged: `feat/storefront-appearance-colours` landed** via `1d65896b2` (PR #2865) — the branch flagged as unmerged in the entry below is now on `develop`.
- **Key finding: two changelog lines are NOT in this clone's `develop` and could not be verified — flagged to the user, docs NOT written for these:**
  - "Adds First-cycle Paddle coupons as one-time discounts" — the enabling commit `91517397b` ("Let a gateway take the first-cycle discount instead of a simulated trial", PR #2857) and its merge `49765084d` exist only on unmerged branches `feat/gateway-first-cycle-discount` / `fix/recurring-discount-issue`, not on `develop`. No `PaymentMethods` file changed on that branch either, so even Paddle's own gateway capability isn't wired up in this clone.
  - "Improves Paddle subscription discount handling" — no merged commit found touching Paddle discount logic in the Sep window; the only Paddle-discount-adjacent commits (`b1a2ab5c3`/`ef3508b3b`, MoR VAT display) predate this range and aren't on `develop` either.
  - The third Paddle-adjacent line, "Improves Recurring coupon handling without simulated trials," IS real and merged (`78207fd86`, part of the same PR #2856 as the two above) — but it's a pure correctness fix (a recurring coupon no longer misfires the one-time-coupon simulated-trial path at checkout) with no new UI, and the existing `adding-coupons/index.md` "What Happens with a One-Time Coupon on a Subscription" section only ever described the one-time-coupon path, so it needed no correction.
- **Key finding: Stripe embedded checkout now inherits the storefront palette.** `fd5da0071` (PR #2873) added `FrontendTheme::stripeAppearance()`/`filterStripeAppearance()`, seeding the `fluent_cart/stripe_appearance` filter. Only fires when Appearance source is `inherit_from_theme` or `customize` AND the palette's `input_bg_color`/`input_text_color` resolve to an actual hex (`ThemePalette::measurable()` rejects unresolved CSS `var()` refs, e.g. Astra/Kadence/GeneratePress). Maps Input background/text → `colorBackground`/`colorText`, Primary background → `colorPrimary`, and switches Stripe's own `theme` to `night` when the input surface is dark (via `ColorMath::readableOn()`).
- **Key finding: customer email verification + guest purchase recovery is a substantial new gate**, not a toggle-able setting (it's a dev filter, `fluent_cart/customer/enable_email_claim`, default on). Merged as `73e570573` (#2883), bundling three sub-fixes: the verification gate itself, `946341202` and `177c1b629` (both preserve customer ownership on email edits — these are also what the changelog's "Guest orders incorrectly linking to existing accounts" and "Customer email changes moving purchase history without verification" Fixes lines refer to, so no separate doc/bug note was needed for those). Recovery is bounded/transactional: synchronous under 5 source customers/100 rows, else a WP-Cron batch job. Full behavior read from `app/Services/CustomerIdentity/*` and the dev plan `dev/CUSTOMER_EMAIL_VERIFICATION_PLAN.md` (used only for orientation, verified against code per the "never trust the spec" rule).
- **Key finding: Compliance is a brand-new top-level settings tab**, not a Store Setup subsection, despite `api/StoreSettings.php` nesting its schema (`compliance` key) at the same array depth as Store Setup's other sections (Pages Setup, Product Page, Cart & checkout, Subscriptions). Confirmed via `resources/admin/Modules/Settings/SettingsView.vue`'s `routes.value.push({...url: '/settings/compliance'})`, a sibling push to Store Settings/Payment Settings/etc, not nested inside them. One setting: `auto_login_after_account_creation` (radio, default `no`), read by both `api/User.php` (direct registration) and `app/Listeners/Order/OrderPaid.php` (guest-account-on-payment), confirmed via diff on `badb39aff` (#2875).
- **Key finding: admin footer feedback link + version** (`e4cc58916`, #2888) is a small cosmetic addition (`app/Modules/AdminFooter/AdminFooter.php`) — WP.org review link on the left, FluentCart (+ Pro if active/different) version on the right, replacing the WordPress version. Documented as a one-line tip in the support page rather than a dedicated section.
- **Doc pages created (sidebar entry added under Settings & Configuration):**
  - `guide/settings-configuration/compliance-settings.md` — Login After Account Creation setting
- **Doc pages updated:**
  - `guide/settings-configuration/appearance.md` — editable hex field description, Stripe embedded-checkout cross-reference
  - `guide/payments-checkout/connecting-payment-gateways/stripe-settings.md` — info callout on embedded checkout inheriting storefront colors (with the CSS-var caveat)
  - `guide/customization-and-themes/code-snippets.md` — new `## Appearance` section (`fluent_cart/stripe_appearance` filter); new subsection under `## Customer Profile` (`fluent_cart/customer/enable_email_claim` filter)
  - `guide/customer-dashboard/index.md` — new "Confirming Your Email Address" section covering the gate and guest purchase recovery
  - `guide/settings-configuration/cart-checkout-settings.md` — cross-reference from User Account Creation Mode to the new Compliance page
  - `guide/settings-configuration/index.md` — Compliance Settings bullet
  - `guide/troubleshooting-support/how-to-get-support.md` — one-line tip pointing to the admin footer for the plugin version
  - `guide/changelog.md` — **NOT touched this session**; the v1.6.5 entry was already committed (`bf4e056`) with the user's pasted text verbatim before this run, including the two unverified Paddle lines above
- **Plugin changes flagged but skipped (no doc impact):** `6f298a1d3` (admin subscription list shows gateway label instead of slug, cosmetic display fix only); "Order bump styling with storefront colors" (no dedicated commit found; order-bump UI already inherits the `--fct-*` variables the Appearance tab writes, so this reads as a natural consequence of the Appearance work already documented, not a separate feature); License lookup performance, PayPal connect-callback hardening (`fb2ba68c9`, predates this range), and the remaining "Fixes" lines (cent-rounding, Stripe submit-error handling, archive pagination count, product video gallery CSS, region translations, order-note role permission, bulk-action button state) — internal correctness/perf fixes with no documented behavior to correct.
- **Screenshots: none taken.** Two genuinely new UI surfaces have no images yet: the Compliance tab (single radio control) and the "Confirm your email address" dashboard notice. A refreshed Appearance screenshot showing the new hex input field would also help but the existing screenshots aren't wrong, just slightly stale. Flagged to the user; offered the `fluentcart-doc-screenshots` pipeline per [[doc-screenshot-pipeline]] rather than manual capture.
- **Open questions carried forward:**
  - Whether to edit or annotate the already-published `guide/changelog.md` v1.6.5 entry given the two unverified Paddle lines — left as-is pending user direction, not edited unilaterally since it's public release copy
  - Confirm with the dev team whether `feat/gateway-first-cycle-discount` was meant to ship in 1.6.5 and got left out, or whether the changelog line is simply premature
  - Sibling branch `feat/storefront-photos-on-submit` still not reviewed (carried forward)
