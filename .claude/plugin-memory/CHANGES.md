...
- **Open questions carried forward:**
  ...
  - **Bricks Blocks addon source still not in the repo.** The 15 blocks are documented from user screenshots, not code, so per-block *options/controls* remain undocumented. Clone the addon (or get its zip) to document block settings properly
